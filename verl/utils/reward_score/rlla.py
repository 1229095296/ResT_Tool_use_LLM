...
def customize_format_reward_func(completions, answer, step, max_possible_reward, min_possible_reward, **kwargs):
    ...
    for response, ans in zip(responses, answer):
        ...
        # Clean response text, remove excess whitespace characters
        response_clean = response.strip()
        
        # Add debug information
        print(f"\n--- Format Check Debug ---")
        print(f"Answer contains response: {'<response>' in ans}")
        print(f"Answer contains tool_call: {'<tool_call>' in ans}")
        print(f"Response contains response: {'<response>' in response_clean}")
        print(f"Response contains tool_call: {'<tool_call>' in response_clean}")
        print(f"Response contains think: {'<think>' in response_clean}")
        
        try:
            if "<response>" in ans and "<tool_call>" not in ans:
                # Case with only response
                if ("<think>" in response_clean and "</think>" in response_clean and 
                    "<response>" in response_clean and "</response>" in response_clean):
                    # Check format: think comes before response
                    think_start = response_clean.find("<think>")
                    think_end = response_clean.find("</think>")
                    response_start = response_clean.find("<response>")
                    response_end = response_clean.find("</response>")
                    
                    if (think_start < think_end < response_start < response_end and
                        response_clean.count("<response>") == 1 and 
                        response_clean.count("</response>") == 1):
                        reward = max_possible_reward
                        
            elif "<response>" not in ans and "<tool_call>" in ans:
                # Case with only tool_call - Note: answer has no response, so response should not have response either
                if ("<think>" in response_clean and "</think>" in response_clean and 
                    "<tool_call>" in response_clean and "</tool_call>" in response_clean and
                    "<response>" not in response_clean and "</response>" not in response_clean):  # Ensure no response tags
                    # Check format: think comes before tool_call
                    think_start = response_clean.find("<think>")
                    think_end = response_clean.find("</think>")
                    tool_start = response_clean.find("<tool_call>")
                    tool_end = response_clean.find("</tool_call>")
                    
                    if (think_start < think_end < tool_start < tool_end and
                        response_clean.count("<tool_call>") == 1 and 
                        response_clean.count("</tool_call>") == 1):
                        reward = max_possible_reward
                        
            elif "<response>" in ans and "<tool_call>" in ans:
                # Case with both tool_call and response
                if ("<think>" in response_clean and "</think>" in response_clean and 
                    "<tool_call>" in response_clean and "</tool_call>" in response_clean and
                    "<response>" in response_clean and "</response>" in response_clean):
                    # Check format: think -> tool_call -> response
                    think_start = response_clean.find("<think>")
                    think_end = response_clean.find("</think>")
                    tool_start = response_clean.find("<tool_call>")
                    tool_end = response_clean.find("</tool_call>")
                    ...
                    if (think_start < think_end < tool_start < tool_end < response_start < response_end and
                        response_clean.count("<tool_call>") == 1 and 
                        response_clean.count("</tool_call>") == 1 and
                        response_clean.count("<response>") == 1 and 
                        response_clean.count("</response>") == 1):
                ...
                # Case with only think
                if ("<think>" in response_clean and "</think>" in response_clean and
                    response_clean.count("<think>") == 1 and 
                    response_clean.count("</think>") == 1):
                    reward = max_possible_reward
...
def compute_score(solution_str, ground_truth, step=0):
    ...
    # Directly modify correctness here, can calculate different rewards based on different steps

    score = correctness_score + fomrat_score
