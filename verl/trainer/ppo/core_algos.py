...
def agg_loss(loss_mat: torch.Tensor, loss_mask: torch.Tensor, loss_agg_mode: str, token_weight: torch.Tensor | None = None):
    """
    ...
    # Modify GSPO to use weighted reward allocation
    elif loss_agg_mode == "seq-mean-token-mean":
        seq_losses = torch.sum(loss_mat * loss_mask * token_weight, dim=-1) / torch.sum(loss_mask, dim=-1)  # token-mean
        loss = torch.mean(seq_losses)  # seq-mean
...
def compute_policy_loss_vanilla(
    ...
    pg_losses = torch.where(advantages < 0, clip_pg_losses2, clip_pg_losses1)
    # === Minor change: Pass through token_weight from batch ===
